function manOWar(input) {
    ...
    for(let i = 0; i < input.length; i++) {
        ...
        switch(command) {
            case "Fire":fire(warship, values[0], values[1]); 
            break;
            case "Defend": defend(pirateShip, values[0], values[1], values[2]);
            break;
            case "Repair": repair(pirateShip, values[0], values[1], maxHealth);
            break;
            case "Status": status(pirateShip, maxHealth);
            break;
            case "Retire": {
